...
function renderizarRecorrentes(recorrentes) {
    const c = document.getElementById('lista-recorrentes'); c.innerHTML = "";
    if(recorrentes.length === 0) return;
    recorrentes.forEach(r => { c.innerHTML += `<div class="history-item" style="border-left: 3px solid var(--fin-color);"><div class="history-details" style="margin-left: 5px;"><div class="history-title">${r.description}</div><div class="history-sub">Vence dia ${r.due_day} • ${r.category}</div></div><div class="history-amount text-danger" style="margin-right: 10px;">R$ ${r.amount.toFixed(2)}</div><button class="btn-small" style="background: rgba(239,68,68,0.15); color:#ef4444;" onclick="deletarRecorrente(${r.id})">${iconTrash}</button></div>`; });
}
...
function renderizarMetasFinanceiras(metas) {
    ...
    metas.forEach(m => {
        ...
        if(m.history && m.history.length > 0) {
            histHtml = `<div style="margin-top: 15px; border-top: 1px solid #2a2c32; padding-top: 10px;"><span style="font-size:11px; color:var(--text-muted); font-weight:bold;">HISTÓRICO:</span><div style="max-height: 80px; overflow-y: auto; margin-top:5px; padding-right:5px;">`;
...
async function salvarMetaSaude() {
    ...
    await fetch(`${API_URL}/goals/health`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(obj) }); fecharModal('modal-meta-saude'); recarregarTudo();
}
...
function renderizarGraficoComida(comidas) {
    ...
    if(barraNutri) {
        barraNutri.style.width = `${avgScore}%`;
        if(avgScore > 75) barraNutri.style.backgroundColor = 'var(--success-color)';
        else if(avgScore > 40) barraNutri.style.backgroundColor = '#f59e0b';
        else barraNutri.style.backgroundColor = 'var(--danger-color)';
    }
