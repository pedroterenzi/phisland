...
def inicializar_banco():
    try:
        ...
        # 🚨 COMANDO DE RESET REMOVIDO DAQUI! Seus dados NUNCA mais serão apagados ao dar F5.
...
# ROTAS AUTH, FINANÇAS E SAÚDE MANTIDAS
...
def transacao_meta(id: int, obj: ModeloMetaTransacao):
    ...
    cursor.execute("INSERT INTO goal_transactions (goal_id, type, amount, description, date) VALUES (%s, %s, %s, %s, %s);", (id, obj.type, obj.amount, obj.description, obj.date)); conn.commit(); cursor.close(); conn.close(); return {"status": "ok"}
...
def listar_tarefas_semana(user_id: int, start: str, end: str):
    conn = psycopg2.connect(DATABASE_URL); cursor = conn.cursor(cursor_factory=RealDictCursor); cursor.execute("SELECT * FROM tasks WHERE user_id = %s AND (is_recurring = TRUE OR (date >= %s AND date <= %s)) ORDER BY time_str ASC, id ASC;", (user_id, start, end)); dados = cursor.fetchall(); cursor.close(); conn.close(); return dados
@app.post("/tasks")
def criar_tarefa(obj: ModeloTarefa):
    conn = psycopg2.connect(DATABASE_URL); cursor = conn.cursor(); cursor.execute("INSERT INTO tasks (user_id, title, date, tag, mental_load, time_str, is_recurring) VALUES (%s, %s, %s, %s, %s, %s, %s);", (obj.user_id, obj.title, obj.date, obj.tag, obj.mental_load, obj.time_str, obj.is_recurring)); conn.commit(); cursor.close(); conn.close(); return {"status": "ok"}
...
# 🚨 ROTA UNIFICADA ATUALIZADA (Recebe a Data de Hoje Exata para buscar o Histórico)
...
def dashboard_unificado(user_id: int, date: str, hoje: str, start_week: str, end_week: str):
    ...
    # 🚨 NOVO: Busca o HISTÓRICO VISUAL dos Treinos de Hoje
    cursor.execute("""
    ...
    for t in cursor.fetchall():
        t_dict = dict(t)
        t_dict['distance_km'] = float(t_dict['distance_km'])
        t_dict['calories_burned'] = float(t_dict['calories_burned'])
        treinos_lista.append(t_dict)
